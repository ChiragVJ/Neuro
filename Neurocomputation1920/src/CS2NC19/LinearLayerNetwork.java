...
/**
 * @author shsmchlr This is a class for a layer of neurons with linear
...
public class LinearLayerNetwork {
	...
	private int weightIndex(int wNeuron, int wWeight) {
		int index;
		if (wWeight == 0) {
			index = ((wNeuron - 1) * (numInputs+1));
		} else {
			index = ((wNeuron - 1) * (numInputs+1) + (wWeight - 1));
		}
		return index; // change this to return the index into weights list of the neuron
	}
	...
	private void changeOneWeight(int wtIndex, double theIn, double delta, double learnRate, double momentum) {
		// write code to update the changeInWeight and the Weight of the wtIndex'th
		// weight
	...
	protected void changeAllWeights(ArrayList<Double> ins, double learnRate, double momentum) {
		...
		for (int i = 1; i <= numNeurons; i++) {
			for (int j = 0; j < (numInputs+1); j++) {
				if (j == 0) {
					theIn = 1.0;
					changeOneWeight(weightIndex(i,j), theIn, deltas.get(i-1), learnRate, momentum);
					
				} else {
					theIn = ins.get(j-1);
					changeOneWeight(weightIndex(i,j+1), theIn, deltas.get(i-1), learnRate, momentum);
	...
	public String getWeights() {
		...
		// write code to add to s suitably
